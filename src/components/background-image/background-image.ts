...
export class BackgroundImage implements OnChanges {
    ...
    ngOnChanges(_changes: { [propName: string]: SimpleChange }) {
        this._update();
    }
    ...
    _update() {
        ...
        img.addEventListener("load", () => {
            this._elementRef.nativeElement.style.backgroundImage = `url(${
                this.src
            })`;
            this._loaded(true);
        });
    ...
    _loaded(isLoaded: boolean) {
        this._elementRef.nativeElement.classList[isLoaded ? "add" : "remove"](
            "img-loaded"
        );
    }
}
