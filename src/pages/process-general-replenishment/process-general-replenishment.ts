...
export class ProcessGeneralReplenishmentPage {
    ...
    async backButtonAction(): Promise<void> {
        try {
            ...
            return await this.navigation.popPage(this.workspace, this.navCtrl, {
    ...
    private async validateLocation(location: string): Promise<boolean> {
        ...
        if (this.taskHeader.Material.batchRequested === Enums.YesNo.Yes) {
            ...
            > = await this.pickingProvider.ValidateIfPickingLicenseIsAvailable(
    ...
    private processMaterialScan(scanData: string): Promise<void> {
        ...
        if (material) {
            let convertedQty = Math.floor(
                this.task.quantityPending / material.QTY
            );
            if (convertedQty <= 0) {
                this.userInteraction.hideLoading();
            ...
            this.processSku.unitMsr = `${material.MEASUREMENT_UNIT} 1x${
                material.QTY
            }`;
            ...
            if (
                ...
                this.navigation.pushPage(
                    Enums.Page.ProcessGeneralReplenishmentSeries,
                    this.workspace,
                    this.navCtrl,
                    {
    ...
    public async userWantsToProcessPicking(): Promise<boolean> {
        try {
            ...
            let dataRequest: DataRequest.RegisterReplenishment = DataRequest.Factory.createRegisterReplenishment(
                this.processSku,
                this.task,
                this.licenseDispatchId,
                this.settings.userCredentials
            );
            let operation: DataResponse.Operation = await this.pickingProvider.registerRePlenishment(
                dataRequest
            );
