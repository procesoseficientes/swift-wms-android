...
export class LocationSuggestionReceptionPage {
    properties: any;
    ...
    useMaterialSubFamily: boolean = false;
    ...
    async ionViewDidEnter() {
        ...
        await this.getParameterSubFamily();
    }
    ...
    async ionViewDidLoad() {
        try {
            ...
            this.generalReceptionParam = <Model.GeneralReceptionParam>this
                .navParams.data;
    ...
    > {
        let suggestedLocation: DataRequest.SuggestedLocation = DataRequest.Factory.createSuggestedLocation(
            this.generalReceptionParam.licenseId,
            this.settings.userCredentials
        );
    ...
    public async processLocationByZone(
        ...
        try {
            ...
            if (this.currentScan === Enums.SuggestedLocationScan.LocationByZone) {
                    ...
                    await this.listSuggestionZone.forEach(
                        async (zone: Model.SuggestionZone) => {
                            ...
                            if (result) {
                                ...
                                if (zone.DIFFERENCE_OF_CLASSES > 0 && zone.MANDATORY == Enums.YesNo.Yes) {
                                    let alertMessage = await this.translate.translateGroupValue(
                                        Enums.Translation.Groups.Alerts,
                                        Enums.Translation.Alert.CreateANewLicenseWithIncompatibleClasses
                                    );
                            ...
                            this.generalReceptionParam.comesFrom = Enums.Page.LocationSuggestion;
                            switch (page) {
                                case Enums.Page.LocateGeneralReceptionLicense:
            ...
            } else if (this.currentScan === Enums.SuggestedLocationScan.Material) {
                // Escaneado en MATERIAL
                var Material = this.listSuggestionMaterial.find(
                    listMaterial => {
    ...
    async validateLocation(location: string): Promise<Enums.Page> {
        ...
        if (validateResult.Resultado === Enums.OperationResult.Success) {
            return Promise.resolve(Enums.Page.LocateGeneralReceptionLicense);
        } else {
            await this.userInteraction.hideLoading();
    ...
    backButtonAction() {
        ...
        return this.navigation.popPage(
            this.workspace,
            this.navCtrl,
            <Model.GeneralReceptionParam>this.generalReceptionParam
        );
    }

    async getParameterSubFamily(): Promise<void> {
        try {
            let requestParameter = DataRequest.Factory.createGetParameterRequest(
                Enums.ParameterGroupId.MaterialSubFamily,
                Enums.ParameterId.UseMaterialSubFamily,
                this.settings.userCredentials
            );
