...
export class LocationSuggestionFullRelocationPage {
    properties: any;
    ...
    async ionViewDidLoad() {
        try {
            ...
            this.relocationFullParam = <Model.GeneralReceptionParam>this
                .navParams.data;
    ...
    > {
        let suggestedLocation: DataRequest.SuggestedLocation = DataRequest.Factory.createSuggestedLocation(
            this.relocationFullParam.licenseId,
            this.settings.userCredentials
        );
                    ...
                    await this.listSuggestionZone.forEach(
                        async (zone: Model.SuggestionZone) => {
                            ...
                            this.relocationFullParam.comesFrom =
                                Enums.Page.LocationSuggestionFullRelocation;
                            switch (page) {
                                case Enums.Page.RelocateFullLicense:
    ...
    async validateLocation(location: string): Promise<Enums.Page> {
        ...
        if (validateResult.Resultado === Enums.OperationResult.Success) {
            return Promise.resolve(Enums.Page.RelocateFullLicense);
        } else {
            await this.userInteraction.hideLoading();
    ...
    backButtonAction() {
        ...
        return this.navigation.popPage(
            this.workspace,
            this.navCtrl,
            <Model.LicenseInfoParams>this.relocationFullParam
        );
    }
}
