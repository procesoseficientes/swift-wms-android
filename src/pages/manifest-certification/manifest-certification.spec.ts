...
describe("ManifesCertificationPage Component", () => { // FIXME: Must split this controller with providers
    let fixture: any;
    let component: ManifestCertificationPage;
    ...
    beforeEach(
        async(() => {
            ...
            TestBed.configureTestingModule({
                declarations: [ManifestCertificationPage, TranslatePipeMock],
                imports: [
                    IonicModule.forRoot(ManifestCertificationPage)
                ],
                providers: [
                    {
    ...
    const loadComponent = () => {
        fixture = TestBed.createComponent(ManifestCertificationPage);
    ...
    it("Should be created", () => {
        return expect(component instanceof ManifestCertificationPage).toBe(true);
    });

    describe("Valid Tests",() =>{
        it("Scanned manifest", () => {
            let execute = async () => {
                let result = await component.getManifest(1);
                expect(component.completedPercentage).toBe(15, "Percentage Certification should be 15");
                return expect(component.certificationId).toBe(1, "Certification header id should be 1");
            };
            return execute();
        });
        ...
        it("Scanned material", () => {
            let execute = async () => {
                component.manifestId = "-1";
                let result = await component.getMaterial("viscosa/VCA1030");

                expect(component.material.materialId).toBe("viscosa/VCA1030", "MaterialId should be the same as the request");
                expect(component.material.materialName).toBe("mock-name", "Material name should be mock-name");
                return expect(result).toBeTruthy("Material scanned must be valid");
            };
            return execute();
        });

        it("Delete Certification detail", () => {
            let execute = async () => {

                await component.deleteCertificationDetail(1,"viscosa/VCA1030");

                return expect(component.completedPercentage).toBe(15, "Percentage Certification should be 15");
            };
            return execute();
        });

        it("keyPressQuantity Insert Certification detail", () => {
            let execute = async () => {
                component.material = Model.Factory.createMaterial();
                component.material.materialId = "viscosa/VCA1030";
                component.material.quantity = 5;
                component.certificationId =  10;
        ...
        it("Mark Certification Complete", () => {
            let execute = async () => {
                component.material = Model.Factory.createMaterial();
                component.material.materialId = "viscosa/VCA1030";
                component.manifestId = "-1";
                component.certificationId =  1;

                await component.markCertificationAsCompleted();

                expect(component.manifestId).toBe("", "ManifestId should be empty value");

                expect(component.material.materialId).toBe("", "MaterialId should be empty value");
                return expect(component.certificationId).toBe(0, "Certification header id should be 0");
            };
            return execute();
        });
    ...
    describe("Invalid Tests", () => {
        ...
        it("Should NOT be able to locate license, result should go through else", () => {
            let execute = async () => {
                await scanEmptyMaterial();
                component.material.quantity = 100;
                ...
                return expect(result).toBeFalsy(
                    "Should NOT be able to locate license, result must be false"
                );
            };
            return execute();
        });

        it("Should NOT be able to locate license, result should go through catch", () => {
            let execute = async () => {
                await scanEmptyMaterial();
                component.material.quantity = 100;
